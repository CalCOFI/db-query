...
export function buildEnvSQL({
  ...
  if (date_min)
    filt.push(`c.datetime_start_utc >= TIMESTAMP '${sqlEsc(date_min)}' - INTERVAL '${pad_hours} hours'`);
  if (date_max)
    filt.push(`c.datetime_start_utc <= TIMESTAMP '${sqlEsc(date_max)}' + INTERVAL '${pad_hours} hours'`);
...
export function buildBioSQLIchthyo({
  ...
  if (date_min) filt.push(`t.datetime_start_utc >= TIMESTAMP '${sqlEsc(date_min)}'`);
  if (date_max) filt.push(`t.datetime_start_utc <= TIMESTAMP '${sqlEsc(date_max)}'`);
