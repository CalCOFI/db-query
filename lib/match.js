...
//
// SQL builders that produce the same DuckDB CTE queries calcofi4r emits as
// `attr(d, "sql")`. The match*() wrappers resolve to { sql, queryMeta } so the
// caller can run sql in DuckDB-WASM (this page), the duckdb CLI, Python, or
// any DuckDB client and get identical rows. The only I/O is the release
// catalog fetch behind readParquetFor() (lib/release.js; cached per version).
//
...
import { readParquetFor, resolveLatestVersion } from "./release.js";
export { resolveLatestVersion };

export const VERSION = "0.4.0";

// Every table is read through the release catalog — readParquetFor(version)
// is the port of calcofi4r's .cc_read_parquet(version): `rp(table)` yields the
// read_parquet(...) expression for that release (content-addressed objects
// since v2026.09, the per-release file before). No URL is concatenated here,
// which is why the builders are async.

// Pull the distinct read_parquet() source URLs out of an emitted SQL string —
// both the single-file form read_parquet('url') and the partitioned list form
// read_parquet(['url1', 'url2'], hive_partitioning = true).
export function extractSourceUrls(sql) {
  const hits = sql.match(/read_parquet\((?:\[[^\]]*\]|'[^']+')/g) || [];
  const urls = hits.flatMap((h) => (h.match(/'[^']+'/g) || []).map((s) => s.slice(1, -1)));
...
// Mirror of calcofi4r::cc_qual_ok_sql(): keep rows whose measurement_qual is not
// suspect/bad/missing in the dataset's own vocabulary (bottle/CTD 8, 9; DIC WOCE
// 3, 4, 9). NULL-safe — an unflagged row is kept; bottle codes were "8.0" through
// v2026.08.14. Vocabulary: workflows/metadata/measurement_qual.csv.
export function qualOkSQL(alias = null) {
  const p = alias ? `${alias}.` : "";
  const q = `regexp_replace(${p}measurement_qual, '\\.0+$', '')`;
  const arms = [
    `(${p}dataset_key IN ('calcofi_bottle', 'calcofi_ctd-cast') AND ${q} IN ('8', '9'))`,
    `(${p}dataset_key = 'calcofi_dic' AND ${p}measurement_qual IN ('3', '4', '9'))`
  ];
  return `COALESCE(NOT (${arms.join(" OR ")}), TRUE)`;
}

// Mirror of .cc_env_sql. `obs` resolves through the catalog: a canonical
// release lists every partition object explicitly, so no directory glob is
// needed over HTTPS.
export async function buildEnvSQL({
  env_var, version,
  depth_m_min = null, depth_m_max = null,
  date_min = null, date_max = null,
  pad_hours = 0
}) {
  const rp = await readParquetFor(version);
...
// standardized tally (std_haul_factor * count / prop_sorted). Tables resolve
// through the catalog (readParquetFor), never a hand-built URL.
export async function buildBioSQLIchthyo({
...
export async function matchIchthyoByName({
  ...
  version = (await readParquetFor(version)).version;   // "latest" → concrete
  ...
  const bio = await buildBioSQLIchthyo({
    version, species_where,
    life_stage, date_min, date_max
  });
  const env = await buildEnvSQL({
    env_var, version,
...
export async function matchIchthyoByTaxon({
...
export async function matchZooplanktonBiomass({
  env_var = "temperature",
